...
interface PulsState {
  weeks: WeekRecord[];
  targetZ2: number;
  updateDay: (weekId: string, dayId: string, type: WorkoutType, minutes: number, avgHeartRate?: number) => void;
  resetData: () => void;
}
...
const createEmptyDays = (weekId: string): DayRecord[] => {
  return DEFAULT_DAYS_LABELS.map((label, idx) => ({
    id: `${weekId}-d${idx}`,
    label,
    state: 'empty',
    minutes: 0,
...
// Set initial test data to match screenshot
INITIAL_WEEKS[0].days[0] = { id: 'w1-d0', label: '37m', state: 'z2', minutes: 37 };
INITIAL_WEEKS[0].days[1] = { id: 'w1-d1', label: '45m', state: 'z2', minutes: 45, isToday: true };
              ...
              const newDays = week.days.map((day) => {
                if (day.id === dayId) {
                   return {
                      ...
                      label: type === 'empty' ? DEFAULT_DAYS_LABELS[parseInt(day.id.split('-d')[1])] : `${minutes}m`,
                   };
                }
                return day;
              });
